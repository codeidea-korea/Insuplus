...
$param["age"] = fn_ins_age(date("Y-m-d",strtotime($birth))); //보험나이
$param["gender"] = $Arr_u_sex[$gender];
...
if($chk_p == "Y") { //단기
	...
	$param["period_day"] = $arr_period["day"];
	$param["period"] = $arr_period["day"];
	$param["period_month"] = $arr_period["month"];
	...
	$arr_period = getArrPeriod($t_s_date,$t_e_date,$chk_p); //기간구하기
	$param["period_day"] = $arr_period["day"];
	$param["period"] = $arr_period["month"];
	$param["period_month"] = $arr_period["month"];
}


$data = getSearchPlanListOfPr_cd($PR_SEQ,$param);
...
if($data) {
		...
		<div class="col-md-3 col-sm-6">
			<div class="list <?=$chk_sel ? "active":"";?>">
				...
				onClick="chkPlan(this,'<?=$row_plan_com["seq"]?>','<?=print_ins($row_plan_com["ins_cd"]);?>','<?=$Arr_plan_cd[$row_plan_com["plan_cd"]]?>','<?=$row_plan_com["chk_service"]?>','<?=print_ins_img($row_plan_com["ins_cd"])?>')"
				<? } else { ?>
