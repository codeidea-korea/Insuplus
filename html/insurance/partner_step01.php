...
require_once '../_nice/checkplus_main_partner.php';
?>
...
&nbsp;&nbsp;&nbsp;※ 자세한 내용은 서비스 약관을 확인해주세요</h4>
            ...
            commonServices.forEach((item, idx) => {
                ...
                if (idx === 0) {
                    ...
                    switch (EHDObject.selectedPartnership[3]) {
                        // case 'myshop':
                        //     temp_title = '의료·여행편의 지원';
                        //     break;
                        case 'rda':
                            temp_title = '이후송 서비스';
                            break;
                        default:
                            temp_title = item.service_group_name;
                    }
                    ...
                    //20240618 이벤트 기간중 보장내역 선택 불가능 처리
                    if (checkEventDate()) {
                        html.push(`      <div class="check-box-inner type01 unclickable" style="display: none;">`);
                        html.push(`  <style>.unclickable {pointer-events: none;}</style>`);
                    } else {
            ...
            if (Array.isArray(optionServices) && optionServices.length > 0) {
                ...
                optionServices.forEach((item, idx) => {
                    if (idx === 0) {
                        ...
                        switch (EHDObject.selectedPartnership[3]) {
                            case 'rda':
                                ...
                                break;
                            default:
                                temp_title = '긴급이후송(자기부담금 20%)';
                        }

                        html.push(`<div class="white-box middle mt24">`);
                        html.push(`  <div class="title-state-box">`);
                        html.push(`    <strong class="memo">${temp_title}</strong>`);
                        html.push(`  </div>`);
                    }
                    ...
                    if (groupName !== item.service_group_name) {
                        ...
                        // html.push(`    <h3>${groupName}</h3>`);
                        html.push(`    <div class="check-box">`);
                        html.push(`      <div class="check-box-inner type02" style="display: none;">`);
                        html.push(`        <input type="checkbox" value="${groupName}" id="service-0${orderNumber}" checked>`);
                        html.push(`        <label for="service-0${orderNumber++}">추가</label>`);
                        html.push(`      </div>`);
                        html.push(`    </div>`);
...
&nbsp;&nbsp;&nbsp;※ 자세한 내용은 서비스 약관을 확인해주세요</h4>
        ...
        kinds.forEach((k, k_idx) => {
            ...
            list.forEach((item, idx) => {
                ...
                if (idx === 0) {
                    ...
                    //20240618 이벤트 기간중 보장내역 선택 불가능 처리
                    if (checkEventDate()) {
                        buff.push(`      <div class="check-box-inner type01 unclickable" style="display: none;">`);
                        html.push(`  <style>.unclickable {pointer-events: none;}</style>`);
                    } else {
                        buff.push(`      <div class="check-box-inner type01" style="display: none;">`);
                    }
                    ///
    ...
    function onClickEventListenerForNextButton(event) {
        const __ = EHDObject;
        const nextPage = './partner_step02.php';
    ...
    function addEventOnDepth1(dataList) {
        if (Array.isArray(dataList) && dataList.length > 0) {
            EHDObject.depth1 = {
                code: dataList[0]['category_code'],
                name: dataList[0]['category_name']
            };
            EHDObject.save();
        } else {
