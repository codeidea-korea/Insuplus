			...
			<div class="sub-content search-wrap">
				<div class='row'>
					<div class='col-sm-10 col-sm-offset-1'>
						...
						<input type="hidden" name="add_gender[]" value="">
							<div class='panel'>
								<div class='panel-body text-md-center text-sm-center text-xs-center'>
								
									<h3 class='text-black'>
										<span class='point'>
											출국 목적과 일정을 선택하고<br/>
											고객님에게 맞는 상품가격을 확인하세요.</span>
									</h3>
										...
										<div class='detail-col-input colspan-3 flex-wrap'>
											<div class='btn-group btn-group-justified btn-group-multi'>
												<? while($row_pr = $dbcon -> fetch_array($RS_PR)){ //상품 검색 ?>
													<div class='btn-group'>
														<a class='btn <?=$row_pr["seq"]==$PR_SEQ ? "btn-theme-bg":"btn-default"?>' 
															ata-toggle='alert-modal' 
															data-size='xs' 
															data-title='알림' 
															data-btn-text='<?=$row_pr["subject"]?>' 
															data-content='<?=$row_pr["ext7"]?>' 
															href='/html/insurance/search_insur.php?PR_SEQ=<?=$row_pr['seq']?>' 
															<? /*if($row_pr['seq'] != $PR_SEQ) { ?> 
															onClick='fn_chg_propose(this,"<?=$row_pr["ext7"]?>")' 
															<? } */?>
															><?=$row_pr["ext7"]?></a>
													</div>
												<? } ?>
									...
									<div class='panel-footer'>
										<a class='btn btn-lg btn-block btn-theme-bg' href="javascript: closedPage();">상품가격 확인 </a>
									</div>
...
function fregist_submit(){
	...
	var diffDate_1 = new Date(sel_s_date.substring(0,4),sel_s_date.substring(4,6),sel_s_date.substring(6,8));
    var diffDate_2 = new Date(sel_e_date.substring(0,4),sel_e_date.substring(4,6),sel_e_date.substring(6,8));
 
    var diff_s_Date_1 =new Date(diffDate_1.getFullYear(), diffDate_1.getMonth()-1, diffDate_1.getDate(),s_date_time);
    var diff_e_Date_2 =new Date(diffDate_2.getFullYear(), diffDate_2.getMonth()-1, diffDate_2.getDate(),e_date_time);

 
    //var diff = Math.abs(diffDate_2.getTime() - diffDate_1.getTime());
	var diff = Math.abs(diff_e_Date_2.getTime() - diff_s_Date_1.getTime());
