...
class INILite
{
	...
	function startAction()
	{
   	...
   	/*--------------------------------------------------*/
		...
		//verify body & headers
		$mtype = $this->m_type;
		if($mtype == "securepay" || $mtype == "receipt"){
			$data_body = $httpclient->getBody();
			$data_header = $httpclient->getHeaders();

			if(strlen($data_body) < 5 || strlen($data_header) < 5){
				$this->MakeErrorMsg(ERR_RESPONSE, "응답전문 수신 실패");
				$this->NetCancel("수신 전문 오류로 인한 망 취소(headers, body)");
				$INILog->CloseLog( $this->m_resultMsg );
				return;
			}
		}
		...
		else
		{
			...
			//망취소
			$this->NetCancel("응답전문 XML 오류로 인한 망취소");
		}

		if( $this->m_resultCode != "00" )
		{
        	$arr = explode("\]+", $this->m_resultMsg);
	...
	//망취소 공용 함수 추가 -- 160429 jhkim
	function NetCancel($netcancel_msg)
	{
		$INIUtil = new INIUtil();
		$INILog = new INILog( $this->m_log, $this->m_debug, $this->m_type );
		$httpclient = new HttpClient( $this->m_ssl );
		 ...
		 )
		{
			...
			if( $this->m_type == "securepay"){
				...
				$this->MakeErrorMsg( ERR_RESPONSE, "취소여부 확인요망");
				return false;
			}
		}
		else
