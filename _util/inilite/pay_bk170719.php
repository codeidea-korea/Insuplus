 ...
 */
    ...
    if( $inipay->m_resultCode == "00" )
    {
        ...
        if($ot_row[orderno]) {
			...
			$row_ord = mysql_fetch_row($result);
...
// 주문여부에 따른 주문선택
			...
			if ($ot_row[member_id]){
				...
				$mdSql = mysql_query($SQL_md,$dbconn);
				$mdRow = mysql_fetch_row($mdSql);
				if ($mdRow[0]){
					// 상담원이 배정된 경우
					$SQL_md2 = "select cs_mid,cs_id,cs_name from cs_admin where cs_id='".$mdRow[0]."' limit 1";
					$mdSql2 = mysql_query($SQL_md2,$dbconn);
					$mdRow2 = mysql_fetch_row($mdSql2);
					$order_md = $mdRow2[0];
					$charge_md = $mdRow2[1];
					...
					$mdSql_ad = mysql_query($SQL_ad,$dbconn);
					$mdRow_ad = mysql_fetch_row($mdSql_ad);
...
//					'NET_A03','NET_A04','NET_A05','NET_A06'	// 7, 8 ,9, 11
					...
					$cs_result = mysql_query($SQL_C,$dbconn);
					}
