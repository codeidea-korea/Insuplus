...
// 이미지가 저장되는 경로
if ( isset($path_editor_upload) ) {
	$dir = $path_editor_upload;
	$dir_url = $url_editor_upload;
}
else {
	$dir = "../../_data/_editor";
	$dir_url  = "../../_data/_editor";
}
...
if ($referer[0] <> $_SERVER['HTTP_HOST']) {
	ECHO "<script>window.alert(editor_lang[71]);</script>";
	exit;
}
...
// 업로드 디렉토리가 있는지 체크
if (!@is_dir($dir)) {
	ECHO "<script>window.alert(editor_lang[72]);</script>";
	exit;
}

// 업로드 폴더의 퍼미션 707인지 체크
if(((substr(PHP_OS,0,1) == 'W') ? 1 : 0) != 1){
	if(substr(decoct(fileperms($dir)),2) <> 707 and substr(decoct(fileperms($dir)),2) <> 777) {
		ECHO "<script>window.alert('".$dir."');</script>";
		ECHO "<script>window.alert(editor_lang[73]);</script>";
		exit;
	}
} // end if
...
if(empty($_POST['wr']) && empty($link) && is_uploaded_file($_FILES['upfile']['tmp_name']) && ($_FILES['upfile']['size'] > 0)) {
	...
	$tmp_dir_url = $dir_url.'/'.(($_POST['mode']==1) ? 'img' : 'mid');
	if(!is_dir($tmp_dir)){
		@mkdir($tmp_dir,0707);
		@chmod($tmp_dir,0707);
	} // end if
	...
	// 미디어이면..
		...
		if($media_chk <> 1){
			ECHO "<script>window.alert(editor_lang[75]);</script>";
			exit;
		}
	} // end if
...
if(is_file($tmp_dir.'/'.$upfile) || !empty($link)){
	...
	$upfile_ok = $tmp_dir_url.'/'.addslashes($upfile);
	...
	// 미디어파일 업로드
		...
		ECHO " autostart=\"true\" loop=\"false\">";
	}
	// 정렬 2-2
	if(!empty($alignment) && ($alignment=='center')){
		ECHO "</div>";
