...
//			"emoticon2|이모티콘2 삽입|viewEmoticonWin()|4",
			...
			"page_add|HTML소스로 삽입|addHtmlCode(event)|4",
	...
	if(toolbar_buttons != "" || source_button != "") {
		image = "whats"; 
		func = "viewInfo(event)"; 
		toolbar_buttons += seperator_str + "<div class='toolbar_unit'><img src=\""+base_dir+"/images/whats.png\" id='spaceditor_btn_editor_"+image+"' class=\"menu_btn\" onMouseOver=\"this.className='menu_btn_up'\" onMouseOut=\"this.className='menu_btn'\" onClick=\""+func+"\" style=\"cursor:pointer\" title=\"Spac Editor는?\" align=\"absmiddle\" width='16' height='16'></div>"; 
		toolbar_buttons2 += seperator_str + "<div class='toolbar_unit'><img src=\""+base_dir+"/images/whats.png\" id='spaceditor_btn_editor_"+image+"' class=\"trans_icon\" title=\"Spac Editor는?\" align=\"absmiddle\" width='16' height='16'></div>";
	}
	toolbar_on_set	 = "";
...
function addSubmitEvent(){
	...
	for(var i = 0; i < document.forms.length; i++) {
		if(	document.forms[i].spaceditor_view_html_source ) {
			form = document.forms[i];
		}
	}
    if(form.addEventListener) {
        form.addEventListener('submit',function OnSubmit() { syncEditor(); }, true);
	} else {
...
function viewFontBoxes(event) {
	var fontbox_table	= "";
	var font_array		=  new Array("굴림","굴림체","돋움","돋움체","바탕","바탕체","궁서","Arial","Tahoma","Verdana");
...
var fontsize_match = new Array();
	fontsize_match["8"] = "36pt";
	fontsize_match["7"] = "24pt";
	fontsize_match["6"] = "18pt";
	fontsize_match["5"] = "14pt";
	fontsize_match["4"] = "12pt";
	fontsize_match["3"] = "10pt";
	fontsize_match["2"] = "9pt";
	fontsize_match["1"] = "8pt";
function viewFontSizeBoxes(event) {
	var fontbox_table	= "";
	var fontsize_array		=  new Array("1","2","3","4","5","6","7","8");
	...
	for(var i = 0; i < fontsize_array.length; i++) {
		fontbox_table	+= "			<a href=\"javascript:setFontSize('"+fontsize_array[i]+"')\"><div style='clear:both;width:auto;padding:3px;background-color:#f2f2f2;color:red;font-size:"+fontsize_match[fontsize_array[i]]+"' onMouseOver=\"this.style.backgroundColor='#d9d9d9'\" onMouseOut=\"this.style.backgroundColor='#f2f2f2'\">안녕!("+fontsize_match[fontsize_array[i]]+")</div></a>";
	}
	fontbox_table		+= "		</td>";
	fontbox_table		+= "	</tr>";
	fontbox_table		+= "</table>";
...
function viewInfo(event) {
	...
	var fontbox_table	= "";
		...
		fontbox_table		+= "	<tr bgcolor='#f2f2f2'>";
		fontbox_table		+= "		<td>";
		fontbox_table		+= "			<div style='font-size:14px;color:black;'>Spac Editor v."+ver+"</div>";
		fontbox_table		+= "			<div style='font-size:11px;color:gray;margin-top:2px;'>Official Site : <a href='http://www.webeditor.kr' target='_blank' title='go website!'>http://www.webeditor.kr</a></div>";
		fontbox_table		+= "			<div style='font-size:11px;color:gray;margin-top:2px;'>Emoticon : <a href='http://http://dlanham.com' target='_blank' title='go website!'>http://dlanham.com</a></div>";
		fontbox_table		+= "			<div style='font-size:11px;color:gray;margin-top:2px;'>Icon : <a href='http://www.famfamfam.com' target='_blank' title='go website!'>http://www.famfamfam.com</a></div>";
		fontbox_table		+= "		</td>";
		fontbox_table		+= "	</tr>";
		fontbox_table		+= "</table>";
		fontbox_table		+= "</div>";
...
function inputTableBox() {
	var obj = document.getElementById('spac_table_box');
	var html = "<div style='clear:both;margin:5px'>"+obj.innerHTML + "</div><p>&nbsp;</p>";
	makeHTML(html);
	hideBlindScreen();
}
...
function uploadImage(form) {
	...
	var upfiles = form.up_file.value.split(".");
...
function addHtmlCode(event) {
	editor.focus();
	getSelectedEditorHtmlText();
	var box_width = 410;
	var table = "";
		table		+= "<div class='property' style='width:auto'>";
		table		+= "<div class='textboxes_title'><img src='"+base_dir+"/images/page_add.png' align='absmiddle' alt='image link window'> HTML소스로 삽입하기</div>";
		table		+= "<div style='float:right;width:auto;padding:3px'><img src='"+base_dir+"/images/btn_close2.png' style='cursor:pointer' onClick='hideBlindScreen()' alt='close property window' title='닫기'></div>";
		table		+= "<div class='textboxes_contents'>";
		table		+= "<table width='100%' border=0 cellpadding=3 cellspacing=1 bgcolor='#d9d9d9'>";
		table		+= "	<tr bgcolor='#f2f2f2'>";
		table		+= "		<td bgcolor='#ffffff'><textarea name='spac_html_content' id='spac_html_content' rows='10' cols='50' style='width:500px;border-width:0' title='HTML소스를 삽입합니다.'></textarea></td>";
		table		+= "	</tr>";
		table		+= "</table>";
		table		+= "<table width='100%'  border=0 cellpadding=5 cellspacing=0>";
		table		+= "	<tr>";
		table		+= "		<td align='center'>";
		table		+= "			<input type='image' id='btn_input_image' src='"+base_dir+"/images/insert_html.png' title='본문에 삽입합니다.' onClick='insertHtmlSource()'>";
		table		+= "		</td>";
		table		+= "	</tr>";
		table		+= "</table>";
		table		+= "</div>";
		...
		var left	 = (getScrollSize('width'))/2-(box_width/2);
		viewBlindScreen(table,event,left);
		document.getElementById('spac_html_content').focus();
}
function insertHtmlSource() {
	makeHTML(document.getElementById('spac_html_content').value);
	hideBlindScreen();
}