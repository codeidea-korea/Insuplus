...
$res = mysql_query("SELECT ci_ww, SUM(ci_todayip) FROM AceMTcounter_ip GROUP BY ci_ww");
while($rs = mysql_fetch_row($res)) {
	$ww = intval($rs[0]);
	$all_hit[$ww] = $rs[1];
}
$res = mysql_query("SELECT ci_ww, SUM(ci_todayip) FROM AceMTcounter_ip {$FindWhere} GROUP BY ci_ww");
while($rs = mysql_fetch_row($res)) {
	$ww = intval($rs[0]);
	$search_hit[$ww] = $rs[1];
}
