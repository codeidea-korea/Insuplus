...
$res = mysql_query("SELECT SUM(cu_hit) FROM AceMTcounter_url");
...
$obj->setTotal("SELECT cu_hit FROM AceMTcounter_url");
$result_rows = $obj->getList("SELECT cu_hit AS now_total, cu_hit*100/$all_total AS per, A.* FROM AceMTcounter_url A ORDER BY {$korder}");
...
<tr height="25" bgcolor="#DDDDDD">
    ...
    <td>
        <b><?=listTitleSort("<font color='#000000'>방문한 URL</font>", "cu_url", "ASC", $nowDir)?></b>
    </td>
    <td width="50" align="right">
        <b><?=listTitleSort("<font color='#000000'>히트</font>", "now_total", "ASC", $nowDir)?></b>
    </td>
    <td width="70" align="center">
        <b><?=listTitleSort("<font color='#000000'>업데이트</font>", "cu_uptime", "ASC", $nowDir)?></b>
    </td>
    <td width="50" align="right">
...
for($i=0; $i<$c; $i++) {
    ...
    $graph = intval($result_rows[$i][now_total] / $all_total * 100);
    if($graph < 1) $graph = 1;
    if($result_rows[$i][cu_url] == "") {
        $result_rows[$i][cu_title] = "즐겨찾기";
    }
    else {
        $result_rows[$i][cu_title] = "<a href='{$result_rows[$i][cu_url]}'>" . cut_string(urldecode($result_rows[$i][cu_url]), 50) . "</a>";
    }
?>

<tr height="25" class="gray">
    <td align="center"><?=$result_rows[$i][_number]?></td>
    <td><?=$result_rows[$i][cu_title]?></td>
    <td align="right"><?=number_format($result_rows[$i][now_total])?></td>
    <td align="center"><?=date("Y-m-d", $result_rows[$i][cu_uptime])?></td>
