<?
include_once("./lib/_util.php");
include_once("./lib/_connect.php");
...
$m1 = substr($date1, 2, 2);
$d1 = substr($date1, 4, 2);
$u1 = $_SERVER['PHP_SELF'] . "?fy=" . $y1 . "&fm=" . $m1 . "&fd=" . $d1 . "&fh=" . $nh;

$date2 = date("ymd", mktime(0,0,0,$m1,$d1-1,$y1));
$y2 = substr($date2, 0, 2);
$m2 = substr($date2, 2, 2);
$d2 = substr($date2, 4, 2);
$u2 = $_SERVER['PHP_SELF'] . "?fy=" . $y2 . "&fm=" . $m2 . "&fd=" . $d2 . "&fh=" . $nh;

$date3 = date("ymd", mktime(0,0,0,$m1,$d1-2,$y1));
$y3 = substr($date3, 0, 2);
$m3 = substr($date3, 2, 2);
$d3 = substr($date3, 4, 2);
$u3 = $_SERVER['PHP_SELF'] . "?fy=" . $y3 . "&fm=" . $m3 . "&fd=" . $d3 . "&fh=" . $nh;

$ul = "fy=" . $_GET['fy'] . "&fm=" . $_GET['fm'] . "&fd=" . $_GET['fd'] . "&fh=" . $_GET['fh'];
...
if($_GET['fd'] != "") {
	$findQuery[] = "ci_dd='$_GET[fd]'";
}
if(count($findQuery)) {
    $FindWhere = " WHERE " . implode(" AND ", $findQuery);
}

$res = mysql_query("SELECT 1 FROM AceMTcounter_ip GROUP BY ci_yy, ci_mm, ci_dd");
    ...
    <td>
        ...
        <tr>
        	<td>
				...
				<select name="fh" onchange="this.form.submit()">
                    <?=selectbox($select_hh, $_GET['fh'], true)?>
                </select>
            </td>
            <td align="center">
            ...
            <td align="right">
                <input type="button" value="오 늘" class="button" onclick="location.href='<?=$u1?>'">
                <input type="button" value="어 제" class="button" onclick="location.href='<?=$u2?>'">
                <input type="button" value="그 제" class="button" onclick="location.href='<?=$u3?>'">
            </td>
        </tr>
        </table>
    </td>
...
<tr>
    <td><img src="img_hour1-1.php?<?=$ul?>"></td>
</tr>
<tr>
    <td align="center">
        [표1-1]. 시간별 방문수 합계
...
<tr>
    <td><img src="img_hour1-2.php?<?=$ul?>"></td>
</tr>
<tr>
    <td align="center">
        [표1-2]. 시간별 평균 방문수
...
<tr>
    <td><img src="img_hour2-1.php?<?=$ul?>"></td>
</tr>
<tr>
    <td align="center">
       [표2-1]. IP당 시간별 방문수 합계
...
<tr>
    <td><img src="img_hour2-2.php?<?=$ul?>"></td>
</tr>
<tr>
    <td align="center">
        [표2-2]. IP당 시간별 평균 방문수
