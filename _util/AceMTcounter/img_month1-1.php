...
$res = mysql_query("SELECT ci_mm, SUM(ci_hit) FROM AceMTcounter_ip GROUP BY ci_mm");
while($rs = mysql_fetch_row($res)) {
	$mm = intval($rs[0]);
	$all_hit[$mm-1] = $rs[1];
}

$res = mysql_query("SELECT ci_mm, SUM(ci_hit) FROM AceMTcounter_ip {$FindWhere} GROUP BY ci_mm");
while($rs = mysql_fetch_row($res)) {
	$mm = intval($rs[0]);
	$search_hit[$mm-1] = $rs[1];
}
