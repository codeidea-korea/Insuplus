...
$obj->setTotal("SELECT ci_hit FROM AceMTcounter_ip WHERE 1 {$findquery}");
$result_rows = $obj->getList("SELECT ci_hit AS now_total, ci_hit*100/$all_total AS per, A.* FROM AceMTcounter_ip A WHERE 1 {$findquery} ORDER BY {$korder}");
