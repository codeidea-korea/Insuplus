<?php
/*
@file KISA_SEED_ECB.php
@brief SEED ECB 암호 알고리즘
...
class Common {
	...
	static function set_byte_for_int(&$dst, $b_offset, $value) {
		if(_KISA_COMMON_IS_LITTLE_ENDIAN ==true) {
			...
			$dst = ($dst & $mask_value2) | ($value2 & $mask_value);
		}
		else {
...
class KISA_SEED_ECB {
	...
	static function Round(&$x, $i7,$i6,$i5,$i4,$i3,$i2,$i1,$i0,$key,$key_offset) {
		$x[$i1] = ($x[$i1] + ((KISA_SEED_ECB::$F1[$x[$i0]] ^ $key[$key_offset+0])&0x0ff)) & 0x0ff;
		$x[$i3] = ($x[$i3] ^ ((KISA_SEED_ECB::$F0[$x[$i2]] + $key[$key_offset+1])&0x0ff)) & 0x0ff;
		$x[$i5] = ($x[$i5] + ((KISA_SEED_ECB::$F1[$x[$i4]] ^ $key[$key_offset+2])&0x0ff)) & 0x0ff;
		$x[$i7] = ($x[$i7] ^ ((KISA_SEED_ECB::$F0[$x[$i6]] + $key[$key_offset+3])&0x0ff)) & 0x0ff;
	}
    ...
    static function SeedRound(&$T, &$LR, $L0, $L1, $R0, $R1, &$K, $K_offset) {		
		...
		$T[1] ^= $T[0];
		$T[1] = KISA_SEED_ECB::$SS0[KISA_SEED_ECB::GetB0($T[1])&0x0ff] ^ KISA_SEED_ECB::$SS1[KISA_SEED_ECB::GetB1($T[1])&0x0ff] ^
				KISA_SEED_ECB::$SS2[KISA_SEED_ECB::GetB2($T[1])&0x0ff] ^ KISA_SEED_ECB::$SS3[KISA_SEED_ECB::GetB3($T[1])&0x0ff];
		$T[0] = ($T[0] + $T[1]) & 0x0ffffffff;
		$T[0] = KISA_SEED_ECB::$SS0[KISA_SEED_ECB::GetB0($T[0])&0x0ff] ^ KISA_SEED_ECB::$SS1[KISA_SEED_ECB::GetB1($T[0])&0x0ff] ^
				KISA_SEED_ECB::$SS2[KISA_SEED_ECB::GetB2($T[0])&0x0ff] ^ KISA_SEED_ECB::$SS3[KISA_SEED_ECB::GetB3($T[0])&0x0ff];
	...
	static function RoundKeyUpdate0(&$T, &$K, $K_offset, &$ABCD, $KC) {
		...
		$T[0] = (($ABCD[KISA_SEED_ECB::$ABCD_A]&0x0ffffffff) + ($ABCD[KISA_SEED_ECB::$ABCD_C]&0x0ffffffff) - ($KC&0x0ffffffff)) & 0x0ffffffff;
		$T[1] = (($ABCD[KISA_SEED_ECB::$ABCD_B]&0x0ffffffff) + ($KC&0x0ffffffff) - ($ABCD[KISA_SEED_ECB::$ABCD_D]&0x0ffffffff)) & 0x0ffffffff;
		$K[$K_offset+0] = KISA_SEED_ECB::$SS0[KISA_SEED_ECB::GetB0($T[0])&0x0ff] ^ KISA_SEED_ECB::$SS1[KISA_SEED_ECB::GetB1($T[0])&0x0ff] ^ KISA_SEED_ECB::$SS2[KISA_SEED_ECB::GetB2($T[0])&0x0ff] ^ KISA_SEED_ECB::$SS3[KISA_SEED_ECB::GetB3($T[0])&0x0ff];
		$K[$K_offset+1] = KISA_SEED_ECB::$SS0[KISA_SEED_ECB::GetB0($T[1])&0x0ff] ^ KISA_SEED_ECB::$SS1[KISA_SEED_ECB::GetB1($T[1])&0x0ff] ^ KISA_SEED_ECB::$SS2[KISA_SEED_ECB::GetB2($T[1])&0x0ff] ^ KISA_SEED_ECB::$SS3[KISA_SEED_ECB::GetB3($T[1])&0x0ff];
		$T[0] = $ABCD[KISA_SEED_ECB::$ABCD_A]; 
		$ABCD[KISA_SEED_ECB::$ABCD_A] = (($ABCD[KISA_SEED_ECB::$ABCD_A]>>8)&0x00ffffff) ^ ($ABCD[KISA_SEED_ECB::$ABCD_B]<<24);
		$ABCD[KISA_SEED_ECB::$ABCD_B] = (($ABCD[KISA_SEED_ECB::$ABCD_B]>>8)&0x00ffffff) ^ ($T[0]<<24);
	}
	...
	static function RoundKeyUpdate1(&$T, &$K, $K_offset, &$ABCD, $KC) {
		$T[0] = (($ABCD[KISA_SEED_ECB::$ABCD_A]&0xFFFFFFFF) + ($ABCD[KISA_SEED_ECB::$ABCD_C]&0xFFFFFFFF) - ($KC&0xFFFFFFFF)) & 0x0ffffffff;
		$T[1] = (($ABCD[KISA_SEED_ECB::$ABCD_B]&0xFFFFFFFF) + ($KC&0xFFFFFFFF) - ($ABCD[KISA_SEED_ECB::$ABCD_D]&0xFFFFFFFF)) & 0x0ffffffff;
		$K[$K_offset+0] = KISA_SEED_ECB::$SS0[KISA_SEED_ECB::GetB0($T[0])&0x0ff] ^ KISA_SEED_ECB::$SS1[KISA_SEED_ECB::GetB1($T[0])&0x0ff] ^ KISA_SEED_ECB::$SS2[KISA_SEED_ECB::GetB2($T[0])&0x0ff] ^ KISA_SEED_ECB::$SS3[KISA_SEED_ECB::GetB3($T[0])&0x0ff];
		$K[$K_offset+1] = KISA_SEED_ECB::$SS0[KISA_SEED_ECB::GetB0($T[1])&0x0ff] ^ KISA_SEED_ECB::$SS1[KISA_SEED_ECB::GetB1($T[1])&0x0ff] ^ KISA_SEED_ECB::$SS2[KISA_SEED_ECB::GetB2($T[1])&0x0ff] ^ KISA_SEED_ECB::$SS3[KISA_SEED_ECB::GetB3($T[1])&0x0ff];
		$T[0] = $ABCD[KISA_SEED_ECB::$ABCD_C];
		$ABCD[KISA_SEED_ECB::$ABCD_C] = ($ABCD[KISA_SEED_ECB::$ABCD_C]<<8) ^ (($ABCD[KISA_SEED_ECB::$ABCD_D]>>24)&0x000000ff);
		$ABCD[KISA_SEED_ECB::$ABCD_D] = ($ABCD[KISA_SEED_ECB::$ABCD_D]<<8) ^ (($T[0]>>24)&0x000000ff);
	}


	static function BLOCK_XOR_ECB(&$OUT_VALUE, $OUT_VALUE_offset, &$IN_VALUE1, $IN_VALUE1_offset, &$IN_VALUE2, $IN_VALUE2_offset) {
	...
	static function KISA_SEED_Encrypt_Block_forECB(&$in, $in_offset, &$out, $out_offset, &$ks) {
		...
		if(_KISA_COMMON_IS_LITTLE_ENDIAN == false) {
			$LR[LR_L0] = KISA_SEED_ECB::EndianChange($LR[LR_L0]);
			$LR[LR_L1] = KISA_SEED_ECB::EndianChange($LR[LR_L1]);
			$LR[LR_R0] = KISA_SEED_ECB::EndianChange($LR[LR_R0]);
			$LR[LR_R1] = KISA_SEED_ECB::EndianChange($LR[LR_R1]);
		}
		
		KISA_SEED_ECB::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K,  0);    // Round 1
		KISA_SEED_ECB::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K,  2);    // Round 2
		KISA_SEED_ECB::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K,  4);    // Round 3
		KISA_SEED_ECB::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K,  6);    // Round 4
		KISA_SEED_ECB::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K,  8);    // Round 5
		KISA_SEED_ECB::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K, 10);    // Round 6
		KISA_SEED_ECB::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K, 12);    // Round 7
		KISA_SEED_ECB::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K, 14);    // Round 8
		KISA_SEED_ECB::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K, 16);    // Round 9
		KISA_SEED_ECB::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K, 18);    // Round 10
		KISA_SEED_ECB::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K, 20);    // Round 11
		KISA_SEED_ECB::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K, 22);    // Round 12
		KISA_SEED_ECB::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K, 24);    // Round 13
		KISA_SEED_ECB::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K, 26);    // Round 14
		KISA_SEED_ECB::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K, 28);    // Round 15
		KISA_SEED_ECB::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K, 30);    // Round 16
	...
	static function KISA_SEED_Decrypt_Block_forECB(&$in, $in_offset, &$out, $out_offset, &$ks ) {
		$LR = array_pad(array(),4,0);		// Iuput/output values at each rounds
		$T = array_pad(array(),2,0);		// Temporary variables for round function F
		$K = &$ks->key_data;				// Pointer of round keys
		...
		KISA_SEED_ECB::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K, 30);    // Round 1
		KISA_SEED_ECB::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K, 28);    // Round 2
		KISA_SEED_ECB::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K, 26);    // Round 3
		KISA_SEED_ECB::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K, 24);    // Round 4
		KISA_SEED_ECB::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K, 22);    // Round 5
		KISA_SEED_ECB::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K, 20);    // Round 6
		KISA_SEED_ECB::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K, 18);    // Round 7
		KISA_SEED_ECB::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K, 16);    // Round 8
		KISA_SEED_ECB::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K, 14);    // Round 9
		KISA_SEED_ECB::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K, 12);    // Round 10
		KISA_SEED_ECB::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K, 10);    // Round 11
		KISA_SEED_ECB::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K,  8);    // Round 12
		KISA_SEED_ECB::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K,  6);    // Round 13
		KISA_SEED_ECB::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K,  4);    // Round 14
		KISA_SEED_ECB::SeedRound($T, $LR, LR_L0, LR_L1, LR_R0, LR_R1, $K,  2);    // Round 15
		KISA_SEED_ECB::SeedRound($T, $LR, LR_R0, LR_R1, LR_L0, LR_L1, $K,  0);    // Round 16
	...
	static function chartoint32_for_SEED_ECB(&$in, $inLen) {
		$data = null;
		$len=0; $i=0;
	...
	static function int32tochar_for_SEED_ECB(&$in, $inLen) {
		$data = null;
		$i = 0;
	...
	// 0xFFFFFFFF 를 & 연산 시키는 이유: 64bit OS에 설치된 PHP의 정수형 데이터 타입의 크기 차이로 인한 오버플로우 처리 오류 
	static function SEED_ECB_init(&$pInfo, $enc, &$pbszUserKey)
	{	
		...
		$ABCD[KISA_SEED_ECB::$ABCD_A] = Common::get_byte_to_int($pbszUserKey, 0);
		$ABCD[KISA_SEED_ECB::$ABCD_B] = Common::get_byte_to_int($pbszUserKey, 4);
		$ABCD[KISA_SEED_ECB::$ABCD_C] = Common::get_byte_to_int($pbszUserKey, 8);
		$ABCD[KISA_SEED_ECB::$ABCD_D] = Common::get_byte_to_int($pbszUserKey, 12);

		if(_KISA_COMMON_IS_LITTLE_ENDIAN == false) {
			$ABCD[KISA_SEED_ECB::$ABCD_A] = KISA_SEED_ECB::EndianChange($ABCD[KISA_SEED_ECB::$ABCD_A]);
			$ABCD[KISA_SEED_ECB::$ABCD_B] = KISA_SEED_ECB::EndianChange($ABCD[KISA_SEED_ECB::$ABCD_B]);
			$ABCD[KISA_SEED_ECB::$ABCD_C] = KISA_SEED_ECB::EndianChange($ABCD[KISA_SEED_ECB::$ABCD_C]);
			$ABCD[KISA_SEED_ECB::$ABCD_D] = KISA_SEED_ECB::EndianChange($ABCD[KISA_SEED_ECB::$ABCD_D]);
		}

		KISA_SEED_ECB::RoundKeyUpdate0($T, $K,  0, $ABCD, KISA_SEED_ECB::$KC0 );    // K_1,0 and K_1,1
		KISA_SEED_ECB::RoundKeyUpdate1($T, $K,  2, $ABCD, KISA_SEED_ECB::$KC1 );    // K_2,0 and K_2,1
		KISA_SEED_ECB::RoundKeyUpdate0($T, $K,  4, $ABCD, KISA_SEED_ECB::$KC2 );    // K_3,0 and K_3,1
		KISA_SEED_ECB::RoundKeyUpdate1($T, $K,  6, $ABCD, KISA_SEED_ECB::$KC3 );    // K_4,0 and K_4,1
		KISA_SEED_ECB::RoundKeyUpdate0($T, $K,  8, $ABCD, KISA_SEED_ECB::$KC4 );    // K_5,0 and K_5,1
		KISA_SEED_ECB::RoundKeyUpdate1($T, $K, 10, $ABCD, KISA_SEED_ECB::$KC5 );    // K_6,0 and K_6,1
		KISA_SEED_ECB::RoundKeyUpdate0($T, $K, 12, $ABCD, KISA_SEED_ECB::$KC6 );    // K_7,0 and K_7,1
		KISA_SEED_ECB::RoundKeyUpdate1($T, $K, 14, $ABCD, KISA_SEED_ECB::$KC7 );    // K_8,0 and K_8,1
		KISA_SEED_ECB::RoundKeyUpdate0($T, $K, 16, $ABCD, KISA_SEED_ECB::$KC8 );    // K_9,0 and K_9,1
		KISA_SEED_ECB::RoundKeyUpdate1($T, $K, 18, $ABCD, KISA_SEED_ECB::$KC9 );    // K_10,0 and K_10,1
		KISA_SEED_ECB::RoundKeyUpdate0($T, $K, 20, $ABCD, KISA_SEED_ECB::$KC10);    // K_11,0 and K_11,1
		KISA_SEED_ECB::RoundKeyUpdate1($T, $K, 22, $ABCD, KISA_SEED_ECB::$KC11);    // K_12,0 and K_12,1
		KISA_SEED_ECB::RoundKeyUpdate0($T, $K, 24, $ABCD, KISA_SEED_ECB::$KC12);    // K_13,0 and K_13,1
		KISA_SEED_ECB::RoundKeyUpdate1($T, $K, 26, $ABCD, KISA_SEED_ECB::$KC13);    // K_14,0 and K_14,1
		KISA_SEED_ECB::RoundKeyUpdate0($T, $K, 28, $ABCD, KISA_SEED_ECB::$KC14);    // K_15,0 and K_15,1

		$T[0] = (($ABCD[KISA_SEED_ECB::$ABCD_A]&0xFFFFFFFF) + ($ABCD[KISA_SEED_ECB::$ABCD_C]&0xFFFFFFFF) - (KISA_SEED_ECB::$KC15&0xFFFFFFFF)) & 0x0ffffffff;
		$T[1] = (($ABCD[KISA_SEED_ECB::$ABCD_B]&0xFFFFFFFF) - ($ABCD[KISA_SEED_ECB::$ABCD_D]&0xFFFFFFFF) + (KISA_SEED_ECB::$KC15&0xFFFFFFFF)) & 0x0ffffffff;
		$K[30] = KISA_SEED_ECB::$SS0[KISA_SEED_ECB::GetB0($T[0])&0x0ff] ^ KISA_SEED_ECB::$SS1[KISA_SEED_ECB::GetB1($T[0])&0x0ff] ^   // K_16,0
				KISA_SEED_ECB::$SS2[KISA_SEED_ECB::GetB2($T[0])&0x0ff] ^ KISA_SEED_ECB::$SS3[KISA_SEED_ECB::GetB3($T[0])&0x0ff];
		$K[31] = KISA_SEED_ECB::$SS0[KISA_SEED_ECB::GetB0($T[1])&0x0ff] ^ KISA_SEED_ECB::$SS1[KISA_SEED_ECB::GetB1($T[1])&0x0ff] ^   // K_16,1
				KISA_SEED_ECB::$SS2[KISA_SEED_ECB::GetB2($T[1])&0x0ff] ^ KISA_SEED_ECB::$SS3[KISA_SEED_ECB::GetB3($T[1])&0x0ff];
	...
	static function SEED_ECB_Process( &$pInfo, &$in, $inLen, &$out, &$outLen ) {
		...
		if( KISA_ENC_DEC::KISA_ENCRYPT == $pInfo->encrypt ) {
			...
			{
				KISA_SEED_ECB::KISA_SEED_Encrypt_Block_forECB( $in, $in_offset, $out, $out_offset, $pInfo->seed_key );
				$nCurrentCount += KISA_SEED_ECB::BLOCK_SIZE_SEED;
				$in_offset += KISA_SEED_ECB::BLOCK_SIZE_SEED_INT;
				$out_offset += KISA_SEED_ECB::BLOCK_SIZE_SEED_INT;
			}

			$outLen = $nCurrentCount - KISA_SEED_ECB::BLOCK_SIZE_SEED;
			$pInfo->buffer_length = $inLen - $outLen;

			Common::arraycopy_system($in, $in_offset, $pInfo->ecb_buffer, 0, $pInfo->buffer_length);
		}
		else {
			$in_offset = 0;
			$out_offset = 0;
			...
			while( $nCurrentCount <= $inLen )
			{
				KISA_SEED_ECB::KISA_SEED_Decrypt_Block_forECB( $in, $in_offset, $out, $out_offset, $pInfo->seed_key );
				$nCurrentCount += KISA_SEED_ECB::BLOCK_SIZE_SEED;
				$in_offset += KISA_SEED_ECB::BLOCK_SIZE_SEED_INT;
				$out_offset += KISA_SEED_ECB::BLOCK_SIZE_SEED_INT;
			}
			$outLen = $nCurrentCount - KISA_SEED_ECB::BLOCK_SIZE_SEED;
			Common::memcpy_int2int( $pInfo->ecb_last_block, $out, $out_offset-4, KISA_SEED_ECB::BLOCK_SIZE_SEED );
		}
	...
	static function SEED_ECB_Close( &$pInfo, &$out, $out_offset, &$outLen ) {
		$nPaddngLeng = null;
		$i = null;
		$outLen = 0;
		...
		if( KISA_ENC_DEC::KISA_ENCRYPT == $pInfo->encrypt ) {
			...
			for( $i = $pInfo->buffer_length; $i<KISA_SEED_ECB::BLOCK_SIZE_SEED; $i++ ) {
				$index = (int)($i/4);
				Common::set_byte_for_int($pInfo->ecb_buffer[$index], $i, $nPaddngLeng);
			}
			KISA_SEED_ECB::KISA_SEED_Encrypt_Block_forECB( $pInfo->ecb_buffer, 0, $out, $out_offset/4, $pInfo->seed_key );
			$outLen = KISA_SEED_ECB::BLOCK_SIZE_SEED;
		}
		else {
			$nPaddngLeng = Common::Get_byte_for_int($pInfo->ecb_last_block, KISA_SEED_ECB::BLOCK_SIZE_SEED-1);
			if( $nPaddngLeng > 0 && $nPaddngLeng <= KISA_SEED_ECB::BLOCK_SIZE_SEED )
			{
				
				for($i = $nPaddngLeng;$i>0;$i--)
				{
					$index = (int)(($out_offset-$i)/4);
					Common::set_byte_for_int($out[$index], $out_offset-$i, 0);
				}
				$outLen = $nPaddngLeng;
			}
			else
...
static function SEED_ECB_Encrypt( &$pbszUserKey, &$message, $message_offset, $message_length ) {
		...
		$nPlainTextPadding = (KISA_SEED_ECB::BLOCK_SIZE_SEED - ($nPlainTextLen % KISA_SEED_ECB::BLOCK_SIZE_SEED));
		...
		$data = KISA_SEED_ECB::chartoint32_for_SEED_ECB($newpbszPlainText, $nPlainTextLen);
		KISA_SEED_ECB::SEED_ECB_Process( $info, $data, $nPlainTextLen, $outbuf, $nRetOutLeng );
		KISA_SEED_ECB::SEED_ECB_Close( $info, $outbuf, $nRetOutLeng, $nPaddingLeng );
		$cdata = KISA_SEED_ECB::int32tochar_for_SEED_ECB($outbuf, $nRetOutLeng+$nPaddingLeng);
	...
	static function SEED_ECB_Decrypt( &$pbszUserKey, &$message, $message_offset, $message_length ) {
		...
		if($nCipherTextLen%KISA_SEED_ECB::BLOCK_SIZE_SEED)
		{
			return $result;
		}
		...
		$outbuf = array_pad(array(), $outlen, 0);
		$data = KISA_SEED_ECB::chartoint32_for_SEED_ECB($newpbszCipherText, $nCipherTextLen);
		KISA_SEED_ECB::SEED_ECB_Process( $info, $data, $nCipherTextLen, $outbuf, $nRetOutLeng );
		if(KISA_SEED_ECB::SEED_ECB_Close( $info, $outbuf, $nRetOutLeng, $nPaddingLeng ))
		{
			$cdata = KISA_SEED_ECB::int32tochar_for_SEED_ECB( $outbuf, $nRetOutLeng-$nPaddingLeng );
