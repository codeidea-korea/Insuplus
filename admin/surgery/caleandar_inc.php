...
function showCalendar( $year, $month, $totalDays, $doctor)
{
	...
	$sSQL = "SELECT name1,r_day, count(name1) as name_cnt  FROM tbl_doctor WHERE r_year  = $year AND r_month =$month group by name1,r_day  order by r_day, name1";
		...
		for( $i = 0; $i < sizeof($arList); $i++) {
			...
			if((int)$arList[$i]["r_day"] == $j) {
				$idx	    = $arList[$i]["idx"];
					...
					$name1 = "<a href='javascript: OnView($year,$month,$j,1)'>".$arList[$i]["name1"]."(".$arList[$i]["name_cnt"].")</a>";
	//천안점
	$sSQL = "SELECT name1,r_day, count(name1) as name_cnt  FROM tbl_doctor WHERE r_year  = $year AND r_month =$month and r_day=$j and t_area=1 and name1='".$arList[$i]["name1"]."' group by name1,r_day  order by r_day, name1";
	$result = mysql_query( $sSQL, $con) or die("DB Error : $sSQL");
	$row = mysql_fetch_array( $result);
					if ($row[name_cnt]){
					$name1 .= "<a href='javascript: OnView($year,$month,$j,1)'><font color='#ff0000'>(".$row[name_cnt].")</font></a>";
					}else{
					$name1 .= "<a href='javascript: OnView($year,$month,$j,1)'><font color='#ff0000'>(0)</font></a>";
					}
	//천안점
	$sSQL = "SELECT name1,r_day, count(name1) as name_cnt  FROM tbl_doctor WHERE r_year  = $year AND r_month =$month and r_day=$j and t_area=2 and name1='".$arList[$i]["name1"]."' group by name1,r_day  order by r_day, name1";
	$result = mysql_query( $sSQL, $con) or die("DB Error : $sSQL");
	$row = mysql_fetch_array( $result);
					if ($row[name_cnt]){
					$name1 .= "<a href='javascript: OnView($year,$month,$j,2)'><font color='#000000'>(".$row[name_cnt].")</font></a>";
					}else{
					$name1 .= "<a href='javascript: OnView($year,$month,$j,2)'><font color='#000000'>(0)</font></a>";
					}
	//천안점
	$sSQL = "SELECT name1,r_day, count(name1) as name_cnt  FROM tbl_doctor WHERE r_year  = $year AND r_month =$month and r_day=$j and t_area=3 and name1='".$arList[$i]["name1"]."' group by name1,r_day  order by r_day, name1";
	$result = mysql_query( $sSQL, $con) or die("DB Error : $sSQL");
	$row = mysql_fetch_array( $result);
					if ($row[name_cnt]){
					$name1 .= "<a href='javascript: OnView($year,$month,$j,3)'><font color='#0000FF'>(".$row[name_cnt].")</font></a>";
					}else{
					$name1 .= "<a href='javascript: OnView($year,$month,$j,3)'><font color='#0000FF'>(0)</font></a>";
					}
		...
		if( $DayInfo != ""){
			$htmlCode .=	"<tr><td colspan=2 align='right' valign='top'>";
			$htmlCode .=	"천 쌍 온<br>";
			$htmlCode .=	"$DayInfo</td></tr>";
		}
...
<table  width="1000" border="0" cellspacing="1" cellpadding="0" bgcolor="#fcdccf" class="mgt10 tx_bod11 c">
					  <tr>
						<td colspan="7" class="tline_bg" height="6"></td>
					  </tr>
					  <tr class="h25" align="center">
						<td class="t_ttbg tx_red11 b c" width="110">일</td>
						<td class="t_ttbg tx_org11 b c" width="110">월</td>
						<td class="t_ttbg tx_org11 b c" width="110">화</td>
						<td class="t_ttbg tx_org11 b c" width="110">수</td>
						<td class="t_ttbg tx_org11 b c" width="110">목</td>
						<td class="t_ttbg tx_org11 b c" width="110">금</td>
						<td class="t_ttbg tx_blue11 b c" width="110">토</td>
					  </tr>
...
function OnView(year,month,day,t_area){
