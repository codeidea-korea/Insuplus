...
<?
	...
	//인슈플러스 감사쿠폰
	$SQL =  " SELECT seq, coupon_name, coupon_size, discount, expire_date_s, expire_date_e ";
	$SQL .= " , (SELECT count(*) FROM tbl_board_coupon_history WHERE event_seq = e.seq ) pub_coupon_size ";
	$SQL .= " , (SELECT count(*) FROM tbl_board_coupon_history WHERE event_seq = e.seq AND use_yn ='Y' ) use_coupon_size ";
	$SQL .= " FROM tbl_board_event e WHERE event_type='C' ";
	$SQL .= " AND event_partnership_code ='insuplus' ORDER BY seq DESC ";
	...
	$field         .= " , e.seq, e.coupon_name, e.discount, e.coupon_size, e.regdate ";
	$field         .= " , e.expire_date_s, e.expire_date_e ";
	$field         .= " , p.partnership_name ";
	$field 		   .= " , (SELECT count(*) FROM tbl_board_coupon_history WHERE event_seq = e.seq ) pub_coupon_size ";
	$field 		   .= " , (SELECT count(*) FROM tbl_board_coupon_history WHERE event_seq = e.seq AND use_yn ='Y' ) use_coupon_size ";
	$table			= " tbl_board_event e LEFT JOIN tbl_board_partner p ";
	$table		   .= " ON e.event_partnership_code = p.partnership_code ";
	$where			= " AND e.event_type = 'C' AND  e.event_partnership_code != 'insuplus' ".$query_where;
	$orderby		= " e.seq DESC ";
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
	<tr>
		...
		<td valign="top" class="a_st">쿠폰</td>
	</tr>
	<tr>
			...
			<? } ?>
				<table class="adm-searchForm">
					...
					<tr>
						...
						<td colspan="2">
							<select name="search">
								<option value="e.coupon_name" <? if ($search == "e.coupon_name" ) echo "selected"; ?>>쿠폰명</option>
			...
			<colgroup>
				...
				<tr>
					...
					<tr>
						<td><?=$coupon_name?></td>
						<td><?=$discount?>%</td>
						<td><?=number_format($coupon_size)?></td>
						<td><?=number_format($pub_coupon_size)?>건</td>
						<td><?=number_format($use_coupon_size)?>건</td>
						<td><input type="button" value="내역확인" onclick="popup_coupon_history_list('<?=$seq?>');" /></td>
					</tr>
					<? }
					unset($RS);
					?>
			...
			<colgroup>
				...
				<?
						...
						while ($rows = $dbcon -> fetch_array($result)) {
							...
							unset($rows);
				?>
					<tr onClick="popup_coupon_history_list('<?=$seq?>')" style="cursor:pointer">
						<td><?=$coupon_name?></td>
...
<script>
	function popup_coupon_history_list(seq) {
		var url = "<?=$url_skin_board?>coupon_history/popup_coupon_history_list.php?event_seq="+seq;
		window.open(url,"_p","width=800,height=600");
	}
