<?
	...
	$field = " a.seq, a.apply_date, a.occur_date , a.gop_status ";
	$field .= " ,j.o_name  ";
...
function excel_go() {
	location.href = "excel_gop.php?mode=excel&<?=$GLOBALS[parameter]?>";
}
...
function WriteGopOkGo() {
	...
	if (!ff.gop_status.value) {
		alert("GOP상태를 선택해 주세요.");
		ff.gop_status.focus();
		return false;
	}
...
<table border="0" cellpadding="0" cellspacing="0" width="100%">
	<tr>
		...
		<td valign="top" class="a_st">gop 관리</td>
	</tr>
	<tr>
...
<table width="100%" cellpadding="0" cellspacing="0" border="0">
	<tr>
		...
		<table class="adm-searchForm">
			...
			<tr>
				...
				<td><select name="pr_cd">
					...
					<select name="gop_status" class="ml10">
						<option value="">GOP상태 선택</option>
						<? foreach($gop_status_arr as $key=>$val) {?>
							<option value="<?=$key;?>" <?=$gop_status==$key ? "selected":"";?>><?=$val;?></option>
						<? } ?>
					</select>
				</td>
			</tr>
			<tr>
				<th>청구일</th>
				<td><input type="text" name="apply_start_date" class="datepicker w100" value="<?=$apply_start_date;?>"> 
					~ <input type="text" name="apply_end_date" class="datepicker w100" value="<?=$apply_end_date;?>">
				</td>
			</tr>
			<tr>
				<th>직접검색</th>
		...
		<tr>
			...
			<td><?=$apply_date;?></td>
			<td><?=$occur_date;?></td>
			<td><?=$gop_status_arr[$gop_status];?></td>
		</tr>
			<? 
			$no--;
			$temp_num++;
