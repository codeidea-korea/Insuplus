...
if($_GET["mode"] == "excel" && getLen($ss_u_idx) > 0) {
	...
	$field = " a.seq, a.apply_date, a.occur_date,a.charge_type, a.charge_status, a.money, a.regdate ";
	$field .= " , j.o_name  ";
	$field .= " , o.s_date, o.e_date, o.pr_name, o.ins_name, o.plan_name, o.chk_service ";
	$field .= " , p.stock_isdn, p.plan_cd  ";
	$field .= " , ( SELECT partnership_name FROM tbl_board_partner WHERE seq = o.join_ch) as parntership_name  ";
	...
	->setCellValue('J1', '신청일')
	->setCellValue('K1', '발생일')
	->setCellValue('L1', '청구유형')
	->setCellValue('M1', '청구상태')
	->setCellValue('N1', '지급 보험료')
	->setCellValue('O1', '등록일');
	...
	if($ArrListRs[0] > 0) {
		while($ListRs = $dbcon -> fetch_array($ArrListRs[1])) {
				...
				->setCellValueExplicit("G{$p}", $parntership_name, PHPExcel_Cell_DataType::TYPE_STRING)
				...
				->setCellValueExplicit("L{$p}", $charge_type, PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("M{$p}", $charge_status, PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("N{$p}", number_format($money), PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("O{$p}", $regdate, PHPExcel_Cell_DataType::TYPE_STRING);
		}
	...
	$objPHPExcel->setActiveSheetIndex(0);
	$file = "insuplus_청구관리_".date("Ymd").".xlsx";
