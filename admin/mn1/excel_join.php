...
if ($_GET["mode"] == "excel" && getLen($ss_u_idx) > 0) {
	...
	$objPHPExcel->setActiveSheetIndex(0)
		...
		// ->setCellValue('J1', '성별')
		->setCellValue('J1', '보험 시작일')
		->setCellValue('K1', '보험 종료일')
		->setCellValue('L1', '보험기간')
		->setCellValue('M1', '보험료')
		->setCellValue('N1', '출국 국가')
		// ->setCellValue('S1', '이메일')
		->setCellValue('O1', '주문번호')
		->setCellValue('P1', 'TID')
		->setCellValue('Q1', '가입상태')
		->setCellValue('R1', '결제상태')
		->setCellValue('S1', '취소일')
		->setCellValue('T1', '상품가격')
		->setCellValue('U1', '할인금액')
		->setCellValue('V1', '결제금액')
		->setCellValue('W1', '추천인코드')
		->setCellValue('X1', '쿠폰코드')
		->setCellValue('Y1', '환불보험료')
		->setCellValue('Z1', '환불서비스료')
		->setCellValue('AA1', '환불금액')
		->setCellValue('AB1', 'B2B/B2C')
		->setCellValue('AC1', '업체명')
		->setCellValue('AD1', '해지일')
		->setCellValue('AE1', '서비스료')
		->setCellValue('AF1', '메모');
	...
	if ($ArrListRs[0] > 0) {
		while ($ListRs = $dbcon->fetch_array($result)) {
			...
			$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValueExplicit("J{$p}", $tol_s_date,							 																PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("K{$p}", $tol_e_date,							 																PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("L{$p}", $tol_ins_period . $arr_chk_p_gubun[$tol_chk_p], 					PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("M{$p}", $tolj_join_amount, 																				PHPExcel_Cell_DataType::TYPE_NUMERIC)
				->setCellValueExplicit("N{$p}", $tol_join_nation_name, 																		PHPExcel_Cell_DataType::TYPE_STRING)
				// ->setCellValueExplicit("P{$p}", $tol_o_email1 . "@" . $tol_o_email2, 											PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("O{$p}", $tolj_orderno, 																						PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("P{$p}", $tol_pg_isdn, 																						PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("Q{$p}", $arr_join_step[$tolj_join_status], 												PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("R{$p}", $arr_ord_step[$tol_order_step], 													PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("S{$p}", $tolj_cancle_date, 																				PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("T{$p}", $tolj_join_amount + $tolj_join_service, 									PHPExcel_Cell_DataType::TYPE_NUMERIC)
				->setCellValueExplicit("U{$p}", $tolj_s_amount, 																					PHPExcel_Cell_DataType::TYPE_NUMERIC)
				->setCellValueExplicit("V{$p}", $tolj_t_amount, 																					PHPExcel_Cell_DataType::TYPE_NUMERIC)
				->setCellValueExplicit("W{$p}", $tbrc_recom_name, 																				PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("X{$p}", $tbe_coupon_name, 																				PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("Y{$p}", $refund_ins_amount, 																			PHPExcel_Cell_DataType::TYPE_NUMERIC)
				->setCellValueExplicit("Z{$p}", $refund_servie_amount,																		PHPExcel_Cell_DataType::TYPE_NUMERIC)
				->setCellValueExplicit("AA{$p}", (int)$refund_amount,																			PHPExcel_Cell_DataType::TYPE_NUMERIC)
				->setCellValueExplicit("AB{$p}", $group_join_type,																				PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("AC{$p}", $client_name,																						PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("AD{$p}", $tol_refund_date,																				PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("AE{$p}", $tolj_join_service,																			PHPExcel_Cell_DataType::TYPE_STRING)
