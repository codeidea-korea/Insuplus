...
if($_GET["mode"] == "excel" && getLen($ss_u_idx) > 0) {
	...
	$field .= " ,a.pr_name, a.plan_name, b.o_name, b.o_isdn1";
	...
	$where .= " AND a.ins_name='현대해상화재보험' ".$query_where;
	$orderby = " a.seq  DESC  ";
	$limit = "";
	...
	->setCellValue('A1', '변경구분')
	->setCellValue('B1', '유형')
	->setCellValue('C1', '주민번호')
	->setCellValue('D1', '성명')
	->setCellValue('E1', '피보험자구분')
	->setCellValue('F1', '상해급수')
	->setCellValue('G1', '개시일')
	->setCellValue('H1', '만기일')
	->setCellValue('I1', '비고')
	->setCellValue('J1', '정산피보험자여부')
	->setCellValue('K1', '주피보험자주민번호')
	->setCellValue('L1', '주피보험자성명')
	->setCellValue('M1', '연봉기준가입여부')
	->setCellValue('N1', '우편번호')
	->setCellValue('O1', '주소')
	->setCellValue('P1', '전화번호')
	->setCellValue('Q1', '휴대폰번호')
	->setCellValue('R1', '이메일주소')
	->setCellValue('S1', '구분')
	->setCellValue('T1', '주민번호정합성 체크구분')
	->setCellValue('U1', '서비스료');
	...
	if($ArrListRs[0] > 0) {
		while($ListRs = $dbcon -> fetch_array($ArrListRs[1])) {
				...
				$p++;
				$objPHPExcel->setActiveSheetIndex(0)
				->setCellValueExplicit("A{$p}", "1", 						PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("B{$p}", "1", 						PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("C{$p}", $birth_date.$o_isdn2,	 	PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("D{$p}", $o_name, 					PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("E{$p}", "5428", 					PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("F{$p}", "3", 						PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("G{$p}", str_replace("-","", $s_date),PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("H{$p}", str_replace("-","", $e_date),PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("I{$p}", "",							PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("J{$p}", "N",						PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("K{$p}", "", 						PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("L{$p}", "", 						PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("M{$p}", "", 						PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("N{$p}", "", 						PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("O{$p}", "", 						PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("P{$p}", "", 						PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("Q{$p}", "", 						PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("R{$p}", "", 						PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("S{$p}", "Z", 						PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("T{$p}", "", 						PHPExcel_Cell_DataType::TYPE_STRING)
				->setCellValueExplicit("U{$p}", $service_amount, PHPExcel_Cell_DataType::TYPE_STRING);
				$objPHPExcel->setActiveSheetIndex(0)->getStyle("U{$p}")->getNumberFormat()->setFormatCode('#,##0');
		}
	...
	$objPHPExcel->setActiveSheetIndex(0);
	$file = "insuplus_현대해상_".date("Ymd").".xlsx";
