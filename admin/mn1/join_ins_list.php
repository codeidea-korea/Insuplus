...
//	echo $query_where."<BR>";
	...
	// $where          .= " AND ins_name= '".$_SESSION["ss_u_name"]."' ";
	$where          .= " AND '".$_SESSION["ss_u_name"]."' = (select y.subject from tbl_board_plan x inner join tbl_board_ins_list y on x.guarantee1_ins_seq = y.seq where x.seq = A.plan_cd limit 1) ";
	$where			.= $query_where;
...
<script>
function view_go(n) {
	location.href = "join_ins_view.php?seq="+n+"<?=$parameter?>";
}
function excel_go() {
	location.href = "excel_join_ins.php?mode=excel&<?=$GLOBALS[parameter]?>";
}
	...
	<tr>
		<td>
			...
			<tr onClick="view_go('<?=$seq?>')" class="click" style="background:<?=$bg?>;">
				...
				<td class="r"><?=number_format($join_amount)?>원</td>
			...
			?>
			<!-- <tr onClick="view_go()" class="click" style="background:#f3c7d5;">
				<td>49</td>
				<td>해외여행상품</td>
				<td>한화손해보험</td>
				<td>레드</td>
				<td>A타입</td>
				<td>2019-09-01 18시</td>
				<td>2019-09-03 18시</td>
				<td>2일</td>
				<td>홍길동</td>
				<td>01012341234</td>
				<td>국민은행</td>
			...
			<tr onClick="view_go()" class="click" style="background:#eee;">
				<td>48</td>
				<td>해외여행상품</td>
				<td>한화손해보험</td>
				<td>레드</td>
