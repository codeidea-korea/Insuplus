  ...
  if($group_join_type === "B2C") { //일반 가입자 인슈플러스 템플릿
    ...
    if($certType != "I") {
      ...
      if ($chk_lang == "E") {
        ...
        $CHK_SERVICE .= "<tr><td><img src=". $signSrc ." align='absmiddle'></td></tr></table>";
      } else {
        //국문 인슈플러스
        ...
        $CHK_SERVICE .=  "  <li>국가 혹은 지역별 ".$brand." 제휴병원은 상황에 따라 수시로 변경될 수 있습니다.</li>";
        $CHK_SERVICE .=  "  <li>이로 인해 병원비 대신지불 서비스가 제공되지 않는 경우 고객이 치료비 납입 후, 보상 청구 하여야 합니다.</li>";
        $CHK_SERVICE .=  "  <li>이 상품은 ".$brand."에서 서비스를 제공하며, 국내 보험사에서 보험담보를 보장합니다.</li>";
        $CHK_SERVICE .=  "  <li>".$brand."는 하기 보험가입증명서의 보험가입금액에 의거하여 서비스를 제공 합니다.</li>";
      ...
      foreach($plan_guarantee_seq as $guarantee_seq){
        ...
        if ( ($row_plan["ext1"] != 'N' && $guarantee_seq == $row_product["ext4"]) || ($row_plan["ext2"] != 'N' && $guarantee_seq == $row_product["ext10"]) ){
          if ($chk_lang == "E") {
            ...
            $G_TABLE .= "Please refer to the ".$brand_e." provides services based on the following collateral and compensation limits.<br/><br/>";
            ...
            $G_TABLE .= "</tbody></table>";
            // 20250327 수정
            $G_TABLE .= "<p style='font-size:12px;margin:8px 0 7px 0;color:#333;'>This insurance contract is a group insurance which ".$brand_e." is policy holder and pays premium.</p>          ";
            ...
            $G_TABLE .= "</tbody></table>";
            $G_TABLE .= "<p style='font-size:12px;margin:8px 0 18px 0;color:#333;'>이 보험계약은 보험계약자 및 보험료 납부자가 ".$brand."인 단체보험으로 예금자보험법에 의거, 예금자보호대상에서 제외됩니다.</p>";
    ...
    if($certType != "I") {
      if ($row_plan["ext3"] != 'N'){
        ...
        if ($chk_fly_type == "A") {
            ...
            $chk_kor_service .= "<p style='font-size:12px;margin:8px 0 5px 0;color:#333;'>".$brand."는 다음의 담보내용과 보상한도액에 의거하여 서비스를 제공합니다.";
      ...
      foreach($plan_guarantee_seq as $guarantee_seq){
        ...
        if ( ($row_plan["ext1"] != 'N' && $guarantee_seq == $row_product["ext4"]) || ($row_plan["ext2"] != 'N' && $guarantee_seq == $row_product["ext10"]) ){
          if ($chk_lang == "E") {
            ...
            //20250327 수정
            $G_TABLE .= "<p style='font-size:12px;margin:8px 0 5px 0;color:#333;'>This insurance contract is a group insurance which ".$brand_e." is policy holder and pays premium.</p>";
