<?
	...
	if ($seq){
		...
		$SQL .= ", (select guarantee1_ins_seq from tbl_board_plan where seq = l.plan_cd limit 1) ins_seq ";
		// $SQL .= " from tbl_order_list l WHERE orderno='".$row_L["orderno"]."' AND ins_name= '".$_SESSION["ss_u_name"]."' ";
...
function fnCertificate() {
	var popCertificate = window.open('popup_ins_certificate.php?seq=<?=$seq?>','popCertificate','top=0,left=0, width=700,height=500');
	...
	<tr>
		<td>
			...
			<table class="adm-view-tb">
				...
				<tr>
					<th>결제상태</th>
					<td><?=$arr_ord_step[$row["order_step"]]?> <?if (trim($row["cancle_date"])!="0000-00-00 00:00:00"){?><span class="txt_red">(결제취소일 : <?=$row["cancle_date"]?>)</span><?}?></td>
					<th>가입상태</th>
