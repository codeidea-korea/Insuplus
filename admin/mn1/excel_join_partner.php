...
if($_GET["mode"] == "excel" && getLen($ss_u_idx) > 0) {
	...
	WHERE 1=1 ";
	$query_where .= " AND a.tol_join_ch='".$_SESSION["ss_partner_seq_admin"]."' ";
	...
	if($ArrListRs[0] > 0) {
		while($ListRs = $dbcon -> fetch_array($result)) {
				...
				$masking_o_isdn2 = substr($ListRs["tolj_o_isdn2"],0,1)."******";
				...
				->setCellValueExplicit("H{$p}", $birth_date."-".$masking_o_isdn2, 					PHPExcel_Cell_DataType::TYPE_STRING)
