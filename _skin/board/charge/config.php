...
//	echo "search_text : ".$search_text."<BR>";
	...
	if ( $bc_search_use == "Y" ) {
		...
		if($pr_cd) {
			$query_where .= " AND o.pr_cd = '".$pr_cd."' ";
		}
		
		if($ins_cd) {
			$query_where .= " AND o.ins_cd = '".$ins_cd."' ";
		}
		
		if($charge_status) {
			$query_where .= " AND charge_status = '".$charge_status."' ";
		}
		...
		if($apply_start_date) {
			$query_where .= " AND date_format(apply_date,'%Y-%m-%d') >= date_format('".$apply_start_date."','%Y-%m-%d') ";
		}
		
		if($apply_end_date) {
			$query_where .= " AND date_format(apply_date,'%Y-%m-%d') <= date_format('".$apply_end_date."','%Y-%m-%d') ";
		}
		
		$parameter .= "&pr_cd=".$pr_cd."&ins_cd=".$ins_cd."&charege_status=".$charege_status;
		$parameter .= "&search=".$search."&search_text=".$search_text;
		$parameter .= "&apply_start_date=".$apply_start_date."&apply_end_date=".$apply_end_date;
