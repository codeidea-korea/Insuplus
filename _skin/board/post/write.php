...
<input type="hidden" name="seq_level" value="<?=$seq_level?>">
						...
						<table>
							...
							<tbody>
								...
								<tr>
									...
									<td><select title="구분선택" id="sel_qna01" class="selectSt01" name="ext1">
									...
									<td>
										...
										<select title="지점 선택" id="sel_qna02" class="selectSt01" name="category">
										<?
											while ($CateListRs = $dbcon -> fetch_array($ArrCateListRs[1]) ) {
										?>
											<option value="<?=$CateListRs[idx]?>" <? if (trim($CateListRs[idx])==trim($bd_category) ) {echo "selected";} ?>><?=$CateListRs[cate_name]?></option>
										<?}?>
										</select>
									</td>
								</tr>
								<tr>
									...
									<td colspan="3">
										<input type="checkbox" name="secret" id="chk_qna01" value="Y" checked/><label for="chk_qna01" class="mL10">기본 비밀글로 등록됩니다.</label>
									</td>
								</tr>
								<tr>
        ...
        if ( $mode == "mod" && getLen($seq) > 0) {
            ...
            while($RowFileRs = $dbcon -> fetch_row($FileRs) ) {
				...
				<tr>
					<td class="m_content">
						<input type='file' name='upfile[]' class='input' style='width:50%;border: 1px solid #d7d7d7;' <? if ( getLen($FC_idx) > 0) { echo "disabled";}?>>
						<?
                if ( getLen($FC_idx) > 0) {
						?>
						<BR>
            ...
            if ( $FileNo == 0) {
				...
				<tr>
					<td class="m_content">
						<input type='file' name='upfile[]' class='input' style='width:50%;border: 1px solid #d7d7d7;padding:0;margin:0;height:18px;'>
					</td>
				</tr>
				<?
            }
				...
				<tr>
					<td class="m_content">
						<input type='file' name='upfile[]' class='input' style='width:50%;border: 1px solid #d7d7d7;padding:0;margin:0;height:18px;line-height: 18px;'>
					</td>
				</tr>
				<?
        }
...
<style>
.file_wrap {position:relative; width:50px; height:24px; vertical-align:middle; display:inline-block; overflow:hidden;}

.file_hidden {position:absolute;right:0;top:0;height:24px;opacity:0;filter:alpha(opacity=0); -ms-filter:"alpha(opacity=0)"; -khtml-opacity:0; -moz-opacity:0; }
</style>
...
<table border=0 cellspacing=0 cellpadding=0 width=100%>
	...
	<?}?>
	<tr>
		<td class="b_txt_w">베스트</td>
		<td class="m_content">
			<input type="checkbox" name="ext3" value="Y" <?if ($ext3=="Y"){echo "checked";}?>> 사용자 화면에서는 체크된 것중 최신순으로 4개가 보여집니다.
		</td>
	</tr>
	<tr>
		<td colspan="2" class="m_line_1px">&nbsp;</td>
