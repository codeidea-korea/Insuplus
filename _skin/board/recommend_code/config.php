<?
	...
	// new 이미지
	$img_new = "<img src='".$url_skin_board.$bc_skin."/images/icon_new.gif' align='absmiddle' border='0'>";

	$img_re = "<img src=\"".$url_skin_board.$bc_skin."/images/icon_re.gif\" align=\"absmiddle\" border=0 />";
	$img_file = "<img src=\"".$url_skin_board.$bc_skin."/images/board_down.gif\" />";
	...
	while ($CateListRs = $dbcon -> fetch_array($ArrPartnerListRs[1]) ) {
		extract($CateListRs);
		$ArrPartnerList[$partnership_code] = $partnership_name;
	}
	$ArrPartnerListRs = $dbcon -> getList($field, $table , $where, $orderby, $limit);
	#############################
	...
	if ( $bc_search_use == "Y" ) {
		...
		if($start_date && $end_date) {
			if($date_type == "regdate") {
				$query_where .= " and ".$date_type." >= '".$start_date."' and ".$date_type." <= '".$end_date."' ";
			} else if($date_type == "partner") {
				$query_where .= " and (	(start_date <= '".$start_date."' and end_date >= '".$start_date."') ";
				$query_where .= " or (start_date >= '".$start_date."' and end_date <= '".$end_date."') ";
				$query_where .= " or (start_date <= '".$end_date."' and end_date >= '".$end_date."')	) ";
			}
			
		}
		
		$parameter .= "&search=".$search."&search_text=".$search_text;
		$parameter .= "&date_type=".$date_type."&start_date=".$start_date."&end_date=".$end_date;
