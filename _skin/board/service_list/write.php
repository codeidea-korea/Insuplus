...
<style>
#service_tb th{text-align:center;padding:3px 0px;border-bottom:1px solid #ededed;}
#service_tb td{text-align:center;border-bottom:1px solid #ededed;}
#service_tb tr:last-child td{border:0px;}
</style>
...
$(function(){
	$("#addTR").click(function () {
		...
		var row = "<tr>";
		row += "<td><input type='text' name='service_name["+length+"]' value='2_1'></td>";
		row += "<td><input type='text' name='service_name_en["+length+"]' value='2_2'></td>";
		row += "<td><input type='radio' name='basic_care["+length+"]' value='Y'> 보장<input type='radio' name='basic_care["+length+"]' value='N'> 미보장</td>";
		row += "<td><input type='radio' name='primium_care["+length+"]' value='Y'> 보장<input type='radio' name='primium_care["+length+"]' value='N'> 미보장</td>";
...
<?
// 서비스 옵션리스트 검색
$SQL_O1 = "select * from tbl_board_service_list_opt where list_seq='".$seq."' ";
...
<textarea name="content" id="content" style="width:100%;height:300px;display:none" class="textarea"><?=$content?>&nbsp;</textarea>
	<tr>
		<td colspan="2" >
			<table width="100%" border="0" id="service_tb">
				<tr>
					<th>서비스내역</th>
					<th>영문서비스내역</th>
					<th>베이직케어</th>
					<th>프리미엄케어</th>
					<th>관리 <em class="inp_black1"><input type="button" value="추가" id="addTR"></em></th>
				</tr>
				<?
				$k = 0;
				...
				<tr>
					...
					<td>
						<input type="radio" name="basic_care[<?=$k?>]" value="Y" <?if ($rows_o1["basic_care"]=="Y"){?>checked<?}?>> 보장
						<input type="radio" name="basic_care[<?=$k?>]" value="N" <?if ($rows_o1["basic_care"]=="N"){?>checked<?}?>> 미보장
					</td>
					<td>
						<input type="radio" name="primium_care[<?=$k?>]" value="Y" <?if ($rows_o1["primium_care"]=="Y"){?>checked<?}?>> 보장
						<input type="radio" name="primium_care[<?=$k?>]" value="N" <?if ($rows_o1["primium_care"]=="N"){?>checked<?}?>> 미보장
					</td>
