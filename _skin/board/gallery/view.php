...
//사용자 모드입니다
	if ($client_mode=="Y"){?>
						<table border="0" cellpadding="0" cellspacing="0" id="board_common" class="text2" >
						<tr>
							<td style="padding:5px 5px 5px 5px;"><?=$subject?></a>
							<td style="padding:5px 5px 5px 5px; width:100px;"><?=$nick_name?></a>
							<td style="padding:5px 5px 5px 5px; width:100px;"><?=$PrintRegDate?></a>
						</tr>
						<tr>
							<td height="1" bgcolor="#63ad0e" colspan="3"></a>
						</tr>
						<tr>
										...
										if ($bc_upfile_image == "Y") {
											...
											if ( getLen($$ObjFileName) > 0 ) {
												...
												for ( $i = 0 ; $i < count(${"Arr_".$ObjFileName}); $i++) {
													...
													echo "<a href=\"javascript:showPicture('".$upload_url."/".${"Arr_".$ObjFileName}[$i][1]."')\"><img src=\"".$upload_url."/".${"Arr_".$ObjFileName}[$i][1]."\" ".${"size".$ObjFileName}." border='0'></a>";
						...
						<table border=0 cellspacing=0 cellpadding=0 width=100%>
							...
							<tr>
								<? if ( $auth_delete ) { ?>
								<td width="70" align="left"><a href="javascript:del_go('<?=$seq?>');"><img src="<?=$url_skin_board.$bc_skin?>/images/b_btn_delete.gif" border="0"></a></td>
								<? } ?>
								<? if ( $auth_modify) { ?>
								<td width="70" align="left"><a href="javascript:mod_go('<?=$seq?>');"><img src="<?=$url_skin_board.$bc_skin?>/images/b_btn_modify.gif" border="0"></a></td>
								<? } ?>
								<? if ( $bc_reply_use == "Y" && $auth_reply ) { ?>
								<td width="70" align="left"><a href="javascript:reply_go('<?=$seq?>');"><img  src="<?=$url_skin_board.$bc_skin?>/images/b_btn_reply.gif" border="0"></a></td>
								<? } ?>
								<? if ( $auth_write) { ?>
								<td width="70" align="left"><a href="javascript:write_go();"><img src="<?=$url_skin_board.$bc_skin?>/images/b_btn_write.gif" border="0"></a></td>
								<? } ?>
								<td align="right"><a href="javascript:list_go();"><img src="<?=$url_skin_board.$bc_skin?>/images/b_btn_list.gif" border="0"></a></td>
							</tr>
						</table>
<?}else{?>
						...
						<table border=0 cellspacing=0 cellpadding=10 width=100%>
							<tr>
								<td valign=top>
									...
									?>
									<img src="/_data/board/<?=$bc_id?>/<?=$ar_filename[0][1]?>.thumb">
