<?
	...
	// new 이미지
	$img_new = "<img src=\"".$url_skin_board.$bc_skin."/images/icon_new.gif\" alt=\"n\" />";

	$img_re = "<img src=\"".$url_skin_board.$bc_skin."/images/icon_re.gif\" align=\"absmiddle\" border=0 />";
	$print_file = "<img src=\"".$url_skin_board.$bc_skin."/images/icon_file.gif\" alt=\"file\" />";
	$img_file = "<img src=\"".$url_skin_board.$bc_skin."/images/icon_file.gif\" alt=\"file\" />";
...
//	echo "search_text : ".$search_text."<BR>";
	...
	if ( $bc_search_use == "Y" ) {
		...
		if($status && $client_mode != "Y") {
			$query_where .= " and status = '".$status."' ";
		}
		
		if($pr_cd) { //상품명
			$query_where .= " and pr_cd = '".$pr_cd."' ";
		}
		
		$parameter .= "&search=".$search."&search_text=".urlencode($search_text);
		$parameter .= "&status=".$status."&pr_cd=".$pr_cd;
